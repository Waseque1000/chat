  ...
  return (
    <div className="flex h-[100dvh] bg-background p-2 md:p-4 lg:p-6 overflow-hidden gap-4 lg:gap-6">
      ...
      <motion.div 
        ...
        className={`w-full md:w-80 lg:w-96 flex-col glass-card rounded-[2rem] z-10 overflow-hidden flex-shrink-0 ${selectedChat ? 'hidden md:flex' : 'flex'}`}
      >
        ...
        <ScrollArea className="flex-1 px-3">
          <div className="space-y-1 py-2 px-2">
            {search.trim() ? (
              ...
              <AnimatePresence>
                <motion.h3 initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xs font-bold text-muted uppercase tracking-widest mb-3 mt-2 px-1">Recent Chats</motion.h3>
                {chats.length === 0 ? (
                  <div className="px-2 py-10 text-center flex flex-col items-center justify-center text-muted">
                        ...
                        className={`relative flex items-center gap-4 p-3 rounded-2xl cursor-pointer transition-all border mb-1 ${
                          isSelected 
                            ...
                            : chat.unread 
                              ...
                              : 'bg-transparent border-transparent hover:bg-tertiary hover:border-border'
                        }`}
                      >
                        <div className="relative">
                          <Avatar className="h-12 w-12 shadow-sm border border-border">
                            <AvatarImage src={getChatAvatar(chat)} />
                            <AvatarFallback className="bg-tertiary text-foreground">{(getChatName(chat) || 'C')[0].toUpperCase()}</AvatarFallback>
                          </Avatar>
                          {isOnline && (
                            <span className="absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full bg-success border-2 border-card shadow-sm"></span>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-baseline mb-0.5">
                            <h4 className={`text-base font-semibold truncate ${isSelected ? 'text-primary' : chat.unread ? 'text-foreground' : 'text-foreground'}`}>{getChatName(chat)}</h4>
                            {chat.latestMessage && (
                              <span className={`text-xs ${chat.unread ? 'text-primary font-bold' : 'text-muted'}`}>
                                {new Date(chat.latestMessage.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                              </span>
                            )}
                          </div>
                          <p className={`text-sm truncate ${chat.unread ? 'text-foreground font-semibold' : 'text-muted'}`}>
                            {chat.latestMessage ? chat.latestMessage.content : "No messages yet"}
                          </p>
                        </div>
                        {chat.unread && !isSelected && (
                          <div className="absolute right-3 top-1/2 -translate-y-1/2 w-3 h-3 bg-primary rounded-full shadow-[0_0_10px_var(--primary)] animate-pulse" />
                        )}</motion.div>
                    )
                  })
                )}
      ...
      <motion.div 
        ...
        className={`flex-1 flex-col relative glass-card rounded-[2rem] overflow-hidden ${!selectedChat ? 'hidden md:flex' : 'flex'}`}
      >
        {selectedChat ? (
          <>
            {/* Chat Header */}
                ...
                {messages.map((m, i) => {
                  ...
                  return (
                    <div key={m._id || i} className={`flex ${isMe ? 'justify-end' : 'justify-start'} group items-end gap-2`}>
                      ...
                      <div className={`max-w-[75%] md:max-w-[65%] rounded-2xl px-4 py-2.5 shadow-sm text-[14px] md:text-[15px] ${
                        isMe 
            ...
            {/* Input Area */}
            <div className="p-3 md:p-6 pt-2 shrink-0 bg-card border-t border-border">
              <form onSubmit={sendMessage} className="flex items-center gap-2 md:gap-3 bg-tertiary p-1.5 md:p-2 rounded-full shadow-sm border border-border">
                <button type="button" className="p-2 md:p-2.5 text-muted hover:text-primary transition-colors rounded-full hover:bg-card ml-0.5 md:ml-1">
                ...
                <Input 
                  ...
                  placeholder="Message..." 
                  className="flex-1 bg-transparent border-transparent focus-visible:ring-0 shadow-none text-[14px] md:text-[15px] text-foreground placeholder:text-muted px-0 h-10"
                />
                <button 
                  type="submit"
                  disabled={!newMessage.trim()}
                  className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-primary-foreground shadow-[0_0_15px_var(--primary)] disabled:opacity-50 disabled:scale-100 hover:scale-105 transition-all mr-0.5 md:mr-1 shrink-0"
                >
